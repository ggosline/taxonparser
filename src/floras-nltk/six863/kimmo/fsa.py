...
from nltk import tokenize, Tree, grammar as CFG
...
def grammar(terminals):
    ...
    rules = [CFG.ProbabilisticProduction(Expr, [Star], prob=0.2),
             CFG.ProbabilisticProduction(Expr, [Plus], prob=0.2),
             CFG.ProbabilisticProduction(Expr, [Qmk], prob=0.2),
             CFG.ProbabilisticProduction(Expr, [Paren], prob=0.2),
             CFG.ProbabilisticProduction(S, [Expr], prob=0.5),
             CFG.ProbabilisticProduction(S, [S, Expr], prob=0.5),
             CFG.ProbabilisticProduction(Star, [Expr, '*'], prob=1),
             CFG.ProbabilisticProduction(Plus, [Expr, '+'], prob=1),
             CFG.ProbabilisticProduction(Qmk, [Expr, '?'], prob=1),
             CFG.ProbabilisticProduction(Paren, ['(', S, ')'], prob=1)]

    prob_term = 0.2 / len(terminals)  # divide remaining pr. mass
    for terminal in terminals:
        rules.append(CFG.ProbabilisticProduction(Expr, [terminal], prob=prob_term))

    return CFG.PCFG(S, rules)
