def sort_subsets(pairs, subsets):
    def subset_size(pair):
        if pair.input() in subsets:
            size1 = len(subsets[pair.input()])
        else:
            size1 = 1
        if pair.output() in subsets:
            size2 = len(subsets[pair.output()])
        else:
            size2 = 1
...
class KimmoPair(object):
    ...
    def make(text):
        parts = text.split(':')
        if len(parts) == 1:
            return KimmoPair(text, text)
        elif len(parts) == 2:
            return KimmoPair(parts[0], parts[1])
        else:
            raise ValueError("Bad format for pair: %s" % text)
