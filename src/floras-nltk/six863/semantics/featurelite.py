...
def unify(feature1, feature2, bindings1=None, bindings2=None, memo=None, fail=None, trace=0):
    ...
    if memo.has_key((id(feature1), id(feature2))):
        ...
        if result is UnificationFailure:
            if trace > 2:
                print
                '(cached) Unifying: %r + %r --> [fail]' % (feature1, feature2)
            raise result()
        if trace > 2:
            print
            '(cached) Unifying: %r + %r --> ' % (feature1, feature2),
            print
            repr(result)
        return result
    ...
    if trace > 1:
        print
        'Unifying: %r + %r --> ' % (feature1, feature2),
    ...
    if trace > 1:
        ...
        print
        'Unifying: %r + %r --> %r' % (feature1, feature2, repr(unified))
...
def _destructively_unify(feature1, feature2, bindings1, bindings2, memo, fail,
    ...
    if depth > 50:
        print
        "Infinite recursion in this unification:"
        print
